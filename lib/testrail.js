...
class TestRail {
  ...
  getPlanId = async (planName, projectId) => {
    try {
  ...
  getSuiteId = async (suiteName, projectId) => {
    try {
  ...
  publishTestResults = async (run, results) => {
    ...
    try {
      const response = await this.addResultsForCases(run.runs[0].id, payload);
  ...
  publishTestRun = async (options, testResults, userAgents) => {
    ...
    if (results.length) {
      const projectId = await this.getProjectId(options.testrailProject);
      const planId = await this.getPlanId(options.testrailPlan, projectId);
      const suiteId = await this.getSuiteId(options.testrailSuite, projectId);

      this.creationDate = this.moment().format("DD_MM_YYYY_HH_mm_ss");

      const agent = userAgents[0].split("/");
      const runName =
        "Run_" + this.creationDate + "(" + agent[0] + "_" + agent[1] + ")";
      const payload = {
        suite_id: suiteId,
        include_all: false,
        case_ids: caseIdList,
      ...
      try {
        const run = await this.addPlanEntry(planId, payload);
