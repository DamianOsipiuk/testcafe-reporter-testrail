  ...
  return {
    ...
    async reportTaskStart(startTime, userAgents, testCount) {
      ...
      this.options = {
        ...
        testrailUser: process.env.TESTRAIL_USER || config.username,
      ...
      if (this.options.testrailEnabled) {
        if (
          ...
          !this.options.testrailProject
        ) {
          this.newline()
            .write(
              this.chalk.red.bold(
                "Error:  TESTRAIL_HOST, TESTRAIL_USER, TESTRAIL_PASS and TESTRAIL_PROJECT must be set as environment variables for the reporter plugin to work correctly"
              )
            )
            .newline();
          process.exit(1);
